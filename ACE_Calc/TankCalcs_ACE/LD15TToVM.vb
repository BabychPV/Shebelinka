...
Public Class LD15TToVM
    ...
    Private PI_______15 As PIACEPoint
    ...
    Private Temperatыra As PIACEPoint
	...
	' Густина15                               PI_______15
	' Маса продукту                           PI_____________
	' Маса продукту корисна                   PI_____________________
	' Мертвий залишок                         PI_______________
	' Обєм продукту                           PI______________2
	' Рівень                                  PI______
	' Стан резервуару                         PI________________2
	' Температура                             PI___________
	' Температура повітря                     PI___________________

    Public Overrides Sub ACECalculations()
        Dim dblLevel, dblVolume, dblTemp, dblAmbTemp, dblD15, dblD, dblMTotal, dblMDead, dblMMax As Double
        Dim dbl_p60, dbl_T, dbl_T15F, dbl_CTPL60, dbl_CTPL, dbl_PSI, dbl_CTSh As Double
        ...
        'Load Density
        If clsFunctions.LoadValueFromACEPoint(Tankis15, Me, dblD15) = False Then
            CalcFailed()
            Exit Sub
        End If
        ...
        dbl_T15F = clsFunctions.mCalcNew.StartFunction("CToF", 15)
        'dbl_PSI = clsFunctions.mCalcNew.StartFunction("PaToPsih", (24.6 * 100))


        dbl_CTPL60 = clsFunctions.mCalcNew.StartFunction("CTPL11ToBaseProduct", dblD15, dbl_T15F, 0, s_ProductGroup)

        dbl_p60 = dblD15 / dbl_CTPL60
        dbl_CTPL = clsFunctions.mCalcNew.StartFunction("CTPL11Product", dbl_p60, dbl_T, 0, s_ProductGroup)
        dbl_CTPL = dbl_CTPL60 / dbl_CTPL
        dblD = dblD15 / dbl_CTPL
    ...
    Protected Overrides Sub InitializePIACEPoints()
        ...
        Tankis15 = GetPIACEPoint("PI_______15")
    ...
    Private Sub CalcFailed()
        ...
        clCalcFailed.Add(Lygis) : clCalcFailed.Add(Tankis15) : clCalcFailed.Add(Mas__Viso) : clCalcFailed.Add(T_ris) _
                : clCalcFailed.Add(Nepaimamas_likutis) : clCalcFailed.Add(AmbTemperature) _
                : clCalcFailed.Add(Temperat_ra) : clCalcFailed.Add(Naudinga_Mas_) : clCalcFailed.Add(Darbinis_Tankis) : clCalcFailed.Add(TankState)
